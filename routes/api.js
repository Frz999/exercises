...
router.post('/users/:_id/exercises', async (req, res) => {
  ...
  if (duration === '') return res.send('duration required')
...
router.get('/users/:_id/logs', async (req, res) => {
  ...
  console.log("QUERY AND PARAMS",req.url)
  try {
    ...
    if (limit && Number(limit) !== NaN) {
      ...
      console.log("QUERY AND PARAMS\n\n",{username: user.username,count: exercises.length,  _id: user._id, log: exercises})

    return res.json({username: user.username,count: exercises.length,  _id: user._id, log: exercises})
  } catch (err) {
